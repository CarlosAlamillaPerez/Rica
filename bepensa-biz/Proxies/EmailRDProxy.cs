...
{
    public class EmailRDProxy : ProxyBase, IAppEmailRD
    {
        private readonly GlobalSettings _ajustes;
        private readonly SmsSettings _smsAjustes;

        public EmailRDProxy(BepensaRD_Context context, IOptionsSnapshot<GlobalSettings> ajustes, IOptionsSnapshot<SmsSettings> smsAjustes)
        {
            DBContextRD = context;
                    ...
                    case TipoMensajeria.Email:
                        ...
                        switch (tipoUsuario)
                        {
                            case TipoUsuario.Usuario:
                                var exec = await DBContextRD.Database.ExecuteSqlRawAsync("EXEC sp_EnvioCorreo_RecuperarPassword @IdUsuario, @Token, @Url, @EnviadoOutPut OUTPUT", parametros);
                                break;
                    ...
                    case TipoMensajeria.Sms:
                        ...
                        switch (tipoUsuario)
                        {
                            case TipoUsuario.Usuario:
                                ...
                                var usuario = DBContextRD.Usuarios.Find(id) ?? throw new Exception(TipoExcepcion.UsuarioNoIdentificado.GetDescription());
        ...
        public void Lectura(Guid? token)
        {
            try
            {
                var query = DBContextRD.BitacoraEnvioCorreos.FirstOrDefault(x => x.Token == token)
        ...
        public Respuesta<string> ObtenerUrlOriginal(string clave)
        {
            ...
            try
            {
                if (!DBContextRD.UrlShorteners.Any(x => x.Clave.Equals(clave) && x.IdEstatus == (int)TipoEstatus.Activo))
                {
                ...
                var url = DBContextRD.UrlShorteners.First(x => x.Clave.Equals(clave) && x.IdEstatus == (int)TipoEstatus.Activo);
